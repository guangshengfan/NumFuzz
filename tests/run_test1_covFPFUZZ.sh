...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    ${ROOT_DIR}/tool/covFPFUZZ/build/bin/afl-clang-fast -g -O0 -fsanitize=address example1.c -o example1AFL
    ...
    ${ROOT_DIR}/tool/covFPFUZZ/build/bin/afl-fuzz -i ${ROOT_DIR}/tests/test1/in -o ${ROOT_DIR}/tests/test1/out_AFL$i -m none -d -t 5000 -- ${ROOT_DIR}/tests/test1/example1AFL @@
fi
