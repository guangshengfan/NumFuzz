...
if ! [ -d "${ROOT_DIR}/tool/NumFuzz/build/bin" ]; then
    ${ROOT_DIR}/tool/install_NumFuzz.sh
fi
...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    ${ROOT_DIR}/tool/NumFuzz/build/bin/afl-clang-fast -g -O0 -fsanitize=address example1.c -o example1AFL
    ...
    ${ROOT_DIR}/tool/NumFuzz/build/bin/afl-fuzz -i ${ROOT_DIR}/tests/test1/in -o ${ROOT_DIR}/tests/test1/out_AFL$i -m none -d -t 5000 -- ${ROOT_DIR}/tests/test1/example1AFL @@
fi
