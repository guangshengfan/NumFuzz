...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    echo "start ..."
    cd ${ROOT_DIR}/specfunc/gsl_sf_hyperg_2F0
    ${ROOT_DIR}/../../tool/AFL-2.52b/build/bin/afl-clang-fast hyperg_2F0.c -o hyperg_2F0 -lm -lgsl -lgslcblas
    if [ -d "${ROOT_DIR}/specfunc/gsl_sf_hyperg_2F0/in"  ]; then
        rm -rf in
    fi
    mkdir in
    echo 0.5 -1.0 > in/testcase
    echo 2.0 0.0 > in/testcase1
    echo 0.981415806270390045363 -2.0 > in/testcase2
    i=0
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${ROOT_DIR}/specfunc/gsl_sf_hyperg_2F0/out_NUMFUZZE$i" ]; then
            break
        fi
    done
    export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
    ${ROOT_DIR}/../../tool/AFL-2.52b/build/bin/afl-fuzz -i ${ROOT_DIR}/specfunc/gsl_sf_hyperg_2F0/in -o ${ROOT_DIR}/specfunc/gsl_sf_hyperg_2F0/out_NUMFUZZE$i -m none -w -t 5000 -- ${ROOT_DIR}/specfunc/gsl_sf_hyperg_2F0/hyperg_2F0 @@
fi
