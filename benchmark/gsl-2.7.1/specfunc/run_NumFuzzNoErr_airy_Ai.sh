...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    echo "start ..."
    cd ${ROOT_DIR}/specfunc/gsl_sf_airy_Ai
    ${ROOT_DIR}/../../tool/AFL-2.52b/build/bin/afl-clang-fast airy_Ai.c -o airy_Ai -lm -lgsl -lgslcblas
    if [ -d "${ROOT_DIR}/specfunc/gsl_sf_airy_Ai/in"  ]; then
        rm -rf in
    fi
    mkdir in
    echo 0.5 > in/testcase
    echo 0.981415806270390045363 > in/testcase1
    echo 2.0 > in/testcase2
    echo -1.0 > in/testcase3
    echo -2.0 > in/testcase4
    echo 3.14159265358979311600e+00 > in/testcase5
    echo -3.14159265358979311600e+00 > in/testcase6
    echo 1.57079632679489655800e+00 > in/testcase7
    echo -1.57079632679489655800e+00 > in/testcase8
    echo 2.71828182845904523536e+00 > in/testcase9
    echo -2.71828182845904523536e+00 > in/testcase10
    i=0
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${ROOT_DIR}/specfunc/gsl_sf_airy_Ai/out_NUMFUZZE$i" ]; then
            break
        fi
    done
    export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
    ${ROOT_DIR}/../../tool/AFL-2.52b/build/bin/afl-fuzz -i ${ROOT_DIR}/specfunc/gsl_sf_airy_Ai/in -o ${ROOT_DIR}/specfunc/gsl_sf_airy_Ai/out_NUMFUZZE$i -m none -t 5000 -- ${ROOT_DIR}/specfunc/gsl_sf_airy_Ai/airy_Ai @@
fi
