...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    echo "start ..."
    cd ${ROOT_DIR}/specfunc/gsl_sf_angle_restrict_symm
    ${ROOT_DIR}/../../tool/AFL-2.52b/build/bin/afl-clang-fast angle_restrict_symm.c -o angle_restrict_symm -lm -lgsl -lgslcblas
    if [ -d "${ROOT_DIR}/specfunc/gsl_sf_angle_restrict_symm/in"  ]; then
        rm -rf in
    fi
    mkdir in
    ...
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${ROOT_DIR}/specfunc/gsl_sf_angle_restrict_symm/out_NUMFUZZE$i" ]; then
            break
        fi
    done
    export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
    ${ROOT_DIR}/../../tool/AFL-2.52b/build/bin/afl-fuzz -i ${ROOT_DIR}/specfunc/gsl_sf_angle_restrict_symm/in -o ${ROOT_DIR}/specfunc/gsl_sf_angle_restrict_symm/out_NUMFUZZE$i -m none -t 5000 -- ${ROOT_DIR}/specfunc/gsl_sf_angle_restrict_symm/angle_restrict_symm @@
fi
