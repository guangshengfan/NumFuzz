...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    ${ROOT_DIR}/../../tool/AFL/build/bin/afl-gcc -fprofile-arcs -ftest-coverage atanint.c -o atanint_cov -lm -lgsl -lgslcblas
fi

afl-cov -d out_AFL2/ --enable-branch-coverage -c . -e "cat AFL_FILE | ./atanint_cov AFL_FILE"
