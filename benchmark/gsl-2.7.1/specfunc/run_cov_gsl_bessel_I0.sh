...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    ${ROOT_DIR}/../../tool/AFL/build/bin/afl-gcc -fprofile-arcs -ftest-coverage bessel_I0.c -o bessel_I0_cov -lm -lgsl -lgslcblas
fi

afl-cov -d out_AFL1/ --enable-branch-coverage -c . -e "cat AFL_FILE | ./bessel_I0_cov AFL_FILE"
