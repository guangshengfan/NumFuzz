...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    echo "start ..."
    cd ${ROOT_DIR}/specfunc/gsl_sf_gamma_inc_Q
    ${ROOT_DIR}/../../tool/AFL-2.52b/build/bin/afl-clang-fast gamma_inc_Q.c -o gamma_inc_Q -fsanitize=address -lm -lgsl -lgslcblas
    if [ -d "${ROOT_DIR}/specfunc/gsl_sf_gamma_inc_Q/in"  ]; then
        rm -rf in
    fi
    mkdir in
    ...
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${ROOT_DIR}/specfunc/gsl_sf_gamma_inc_Q/out_NUMFUZZ$i" ]; then
            break
        fi
    done
    export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
    ${ROOT_DIR}/../../tool/AFL-2.52b/build/bin/afl-fuzz -i ${ROOT_DIR}/specfunc/gsl_sf_gamma_inc_Q/in -o ${ROOT_DIR}/specfunc/gsl_sf_gamma_inc_Q/out_NUMFUZZ$i -m none -w -t 5000 -- ${ROOT_DIR}/specfunc/gsl_sf_gamma_inc_Q/gamma_inc_Q @@
fi
