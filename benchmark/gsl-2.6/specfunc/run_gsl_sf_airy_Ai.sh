...
#    echo 0.999999999999999888978 > in/testcase3
    echo 2.0 > in/testcase4
#    echo 1.99999999999999977796 > in/testcase5
#    echo 3.99999999999999955591 > in/testcase6
#    echo 3.92566322508156018145 > in/testcase7
    i=0
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${ROOT_DIR}/specfunc/gsl_sf_airy_Ai/out_AFL$i" ]; then
            break
        fi
    done
    export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
    ${ROOT_DIR}/../../tool/AFL-2.52b/build/bin/afl-fuzz -i ${ROOT_DIR}/specfunc/gsl_sf_airy_Ai/in -o ${ROOT_DIR}/specfunc/gsl_sf_airy_Ai/out_AFL$i -m none -t 5000 -- ${ROOT_DIR}/specfunc/gsl_sf_airy_Ai/airy_Ai @@
fi
