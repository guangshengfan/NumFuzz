...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    echo "start ..."
    cd ${ROOT_DIR}/FUZZ/fdlibm
    ${ROOT_DIR}/../tool/AFL-2.52b/build/bin/afl-clang-fast e_acos.c -o e_acos -lm -lgsl -lgslcblas
    if [ -d "${ROOT_DIR}/FUZZ/fdlibm/in"  ]; then
        rm -rf in
    fi
    mkdir in
...
#    echo 3.92566322508156018145 > in/testcase7
    ...
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${ROOT_DIR}/FUZZ/fdlibm/out_AFL$i" ]; then
            break
        fi
    done
    export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
    ${ROOT_DIR}/../tool/AFL-2.52b/build/bin/afl-fuzz -i ${ROOT_DIR}/FUZZ/fdlibm/in -o ${ROOT_DIR}/FUZZ/fdlibm/out_AFL$i -m none -t 5000 -- ${ROOT_DIR}/FUZZ/fdlibm/e_acos @@
fi
