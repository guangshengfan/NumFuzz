...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    echo "start ..."
    cd ${ROOT_DIR}/AFL_cov/gsl_sf_bessel_I1_scaled
    ${ROOT_DIR}/../tool/AFL/build/bin/afl-clang-fast -Wl,-rpath=/usr/local/lib -lgsl -lgslcblas -lm bessel_I1_scaled.c -o bessel_I1_scaled
    if [ -d "${ROOT_DIR}/AFL_cov/gsl_sf_bessel_I1_scaled/in"  ]; then
        rm -rf in
    fi
    mkdir in
    ...
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${ROOT_DIR}/AFL_cov/gsl_sf_bessel_I1_scaled/out_covAFL$i" ]; then
            break
        fi
    done
    export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
    ${ROOT_DIR}/../tool/AFL/build/bin/afl-fuzz -i ${ROOT_DIR}/AFL_cov/gsl_sf_bessel_I1_scaled/in -o ${ROOT_DIR}/AFL_cov/gsl_sf_bessel_I1_scaled/out_covAFL$i -m none -t 5000 -- ${ROOT_DIR}/AFL_cov/gsl_sf_bessel_I1_scaled/bessel_I1_scaled @@
fi
