...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    ${ROOT_DIR}/../../tool/AFL/build/bin/afl-gcc -fprofile-arcs -ftest-coverage s_nextafter.c -o s_nextafter_cov -lm -lgsl -lgslcblas
fi

afl-cov -d out_covAFL1/ --enable-branch-coverage -c . -e "cat AFL_FILE | ./s_nextafter_cov AFL_FILE"
