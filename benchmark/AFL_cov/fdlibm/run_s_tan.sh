...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    ${ROOT_DIR}/../../tool/AFL/build/bin/afl-clang-fast s_tan.c -o s_tan -lm -lgsl -lgslcblas
    ...
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${ROOT_DIR}/fdlibm/s_tan/out_covAFL$i" ]; then
            break
        fi
    done
    export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
    ${ROOT_DIR}/../../tool/AFL/build/bin/afl-fuzz -i ${ROOT_DIR}/fdlibm/s_tan/in -o ${ROOT_DIR}/fdlibm/s_tan/out_covAFL$i -m none -t 5000 -- ${ROOT_DIR}/fdlibm/s_tan/s_tan @@
fi
