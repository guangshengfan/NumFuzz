...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    ${ROOT_DIR}/../../tool/AFL/build/bin/afl-clang-fast e_sinh.c -o e_sinh -lm -lgsl -lgslcblas
    ...
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${ROOT_DIR}/fdlibm/e_sinh/out_covAFL$i" ]; then
            break
        fi
    done
    export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
    ${ROOT_DIR}/../../tool/AFL/build/bin/afl-fuzz -i ${ROOT_DIR}/fdlibm/e_sinh/in -o ${ROOT_DIR}/fdlibm/e_sinh/out_covAFL$i -m none -t 5000 -- ${ROOT_DIR}/fdlibm/e_sinh/e_sinh @@
fi
