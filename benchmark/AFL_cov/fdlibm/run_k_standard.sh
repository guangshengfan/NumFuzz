...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    echo "start ..."
    cd ${ROOT_DIR}/fdlibm/k_standard
    ${ROOT_DIR}/../../tool/covFPFUZZ/build/bin/afl-clang-fast k_standard.c -o k_standard -lm
    if [ -d "${ROOT_DIR}/fdlibm/k_standard/in"  ]; then
        rm -rf in
    fi
    mkdir in
    ...
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${ROOT_DIR}/fdlibm/k_standard/out_covFPFUZZ$i" ]; then
            break
        fi
    done
    export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
    ${ROOT_DIR}/../../tool/covFPFUZZ/build/bin/afl-fuzz -i ${ROOT_DIR}/fdlibm/k_standard/in -o ${ROOT_DIR}/fdlibm/k_standard/out_covFPFUZZ$i -m none -e -t 5000 -- ${ROOT_DIR}/fdlibm/k_standard/k_standard @@
fi
