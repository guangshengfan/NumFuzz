...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    ${ROOT_DIR}/../tool/covFPFUZZ/build/bin/afl-gcc -fprofile-arcs -ftest-coverage sin.c -o sin_cov -lm -lgsl -lgslcblas
fi

afl-cov -d out_covFPFUZZ1/ --enable-branch-coverage -c . -e "cat AFL_FILE | ./sin_cov AFL_FILE"
