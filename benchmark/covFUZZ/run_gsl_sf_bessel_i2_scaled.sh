...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    echo "start ..."
    cd ${ROOT_DIR}/covFUZZ/gsl_sf_bessel_i2_scaled
    ${ROOT_DIR}/../tool/covFPFUZZ/build/bin/afl-clang-fast -Wl,-rpath=/usr/local/lib -lgsl -lgslcblas -lm i2scaled.c -o i2scaled
    if [ -d "${ROOT_DIR}/covFUZZ/gsl_sf_bessel_i2_scaled/in"  ]; then
        rm -rf in
    fi
    mkdir in
...
#    echo 0.981415806270390045363 > in/testcase2
    ...
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${ROOT_DIR}/covFUZZ/gsl_sf_bessel_i2_scaled/out_covFPFUZZ$i" ]; then
            break
        fi
    done
    export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
    ${ROOT_DIR}/../tool/covFPFUZZ/build/bin/afl-fuzz -i ${ROOT_DIR}/covFUZZ/gsl_sf_bessel_i2_scaled/in -o ${ROOT_DIR}/covFUZZ/gsl_sf_bessel_i2_scaled/out_covFPFUZZ$i -m none -t 5000 -- ${ROOT_DIR}/covFUZZ/gsl_sf_bessel_i2_scaled/i2scaled @@
fi
