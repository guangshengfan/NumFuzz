...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    echo "start ..."
    cd ${ROOT_DIR}/fdlibm/e_acos
    ${ROOT_DIR}/../../tool/covFPFUZZ/build/bin/afl-clang-fast e_acos.c -o e_acos -lm -lgsl -lgslcblas
    if [ -d "${ROOT_DIR}/fdlibm/e_acos/in"  ]; then
        rm -rf in
    fi
    mkdir in
    echo 1.0 > in/testcase
    echo 0.981415806270390045363 > in/testcase1
    echo 0.3 > in/testcase2
    echo 0.0 > in/testcase3
    echo inf > in/testcase4
    echo nan > in/testcase5
    echo -1.0 > in/testcase6
    echo -2.0 > in/testcase7 
    i=0
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${ROOT_DIR}/fdlibm/e_acos/out_covFPFUZZ$i" ]; then
            break
        fi
    done
    export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
    ${ROOT_DIR}/../../tool/covFPFUZZ/build/bin/afl-fuzz -i ${ROOT_DIR}/fdlibm/e_acos/in -o ${ROOT_DIR}/fdlibm/e_acos/out_covFPFUZZ$i -m none -t 5000 -- ${ROOT_DIR}/fdlibm/e_acos/e_acos @@
fi
