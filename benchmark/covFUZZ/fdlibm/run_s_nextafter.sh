...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    echo "start ..."
    cd ${ROOT_DIR}/fdlibm/s_nextafter
    ${ROOT_DIR}/../../tool/covFPFUZZ/build/bin/afl-clang-fast s_nextafter.c -o s_nextafter -lm -lgsl -lgslcblas
    if [ -d "${ROOT_DIR}/fdlibm/s_nextafter/in"  ]; then
        rm -rf in
    fi
    mkdir in
    echo 0.5  > in/testcase
    echo 0.981415806270390045363 0.0 > in/testcase1
    echo 0.0 2.0 > in/testcase2
    echo nan 0.0 > in/testcase3
    echo inf 1.0 > in/testcase4
    i=0
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${ROOT_DIR}/fdlibm/s_nextafter/out_covFPFUZZ$i" ]; then
            break
        fi
    done
    export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
    ${ROOT_DIR}/../../tool/covFPFUZZ/build/bin/afl-fuzz -i ${ROOT_DIR}/fdlibm/s_nextafter/in -o ${ROOT_DIR}/fdlibm/s_nextafter/out_covFPFUZZ$i -m none -w -t 5000 -- ${ROOT_DIR}/fdlibm/s_nextafter/s_nextafter @@
fi
