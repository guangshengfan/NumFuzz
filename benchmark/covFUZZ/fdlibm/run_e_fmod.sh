...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    echo "start ..."
    cd ${ROOT_DIR}/fdlibm/e_fmod
    ${ROOT_DIR}/../../tool/covFPFUZZ/build/bin/afl-clang-fast e_fmod.c -o e_fmod -lm -lgsl -lgslcblas
    if [ -d "${ROOT_DIR}/fdlibm/e_fmod/in"  ]; then
        rm -rf in
    fi
    mkdir in
    echo 0.0 1.0> in/testcase
    echo 0.981415806270390045363 nan > in/testcase1
    echo 1.0  inf > in/testcase2
    echo nan 1.0 > in/testcase3
    echo inf 0.0 > in/testcase4
    ...
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${ROOT_DIR}/fdlibm/e_fmod/out_covFPFUZZ$i" ]; then
            break
        fi
    done
    export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
    ${ROOT_DIR}/../../tool/covFPFUZZ/build/bin/afl-fuzz -i ${ROOT_DIR}/fdlibm/e_fmod/in -o ${ROOT_DIR}/fdlibm/e_fmod/out_covFPFUZZ$i -m none -w -t 5000 -- ${ROOT_DIR}/fdlibm/e_fmod/e_fmod @@
fi
