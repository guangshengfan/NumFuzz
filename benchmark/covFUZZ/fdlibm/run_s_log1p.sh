...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    echo "start ..."
    cd ${ROOT_DIR}/fdlibm/s_log1p
    ${ROOT_DIR}/../../tool/covFPFUZZ/build/bin/afl-clang-fast s_log1p.c -o s_log1p -lm -lgsl -lgslcblas
    if [ -d "${ROOT_DIR}/fdlibm/s_log1p/in"  ]; then
        rm -rf in
    fi
    mkdir in
    ...
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${ROOT_DIR}/fdlibm/s_log1p/out_covFPFUZZ$i" ]; then
            break
        fi
    done
    export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
    ${ROOT_DIR}/../../tool/covFPFUZZ/build/bin/afl-fuzz -i ${ROOT_DIR}/fdlibm/s_log1p/in -o ${ROOT_DIR}/fdlibm/s_log1p/out_covFPFUZZ$i -m none -t 5000 -- ${ROOT_DIR}/fdlibm/s_log1p/s_log1p @@
fi
