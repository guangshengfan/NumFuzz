...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    echo "start ..."
    cd ${ROOT_DIR}/fdlibm/e_atan2
    ${ROOT_DIR}/../../tool/covFPFUZZ/build/bin/afl-clang-fast e_atan2.c -o e_atan2 -lm -lgsl -lgslcblas
    if [ -d "${ROOT_DIR}/fdlibm/e_atan2/in"  ]; then
        rm -rf in
    fi
    mkdir in
    echo 0.5 > in/testcase
    echo 0.981415806270390045363 nan > in/testcase1
    echo 2.0  inf > in/testcase2
    echo 0.0 1.0 > in/testcase3
    echo inf 0.0 > in/testcase4
    i=0
    for ((i=1; i<100; i++))
    do
        if ! [ -d "${ROOT_DIR}/fdlibm/e_atan2/out_covFPFUZZ$i" ]; then
            break
        fi
    done
    export ASAN_OPTIONS=detect_odr_violation=0:allocator_may_return_null=0:abort_on_error=1:symbolize=0:detect_leaks=0
    ${ROOT_DIR}/../../tool/covFPFUZZ/build/bin/afl-fuzz -i ${ROOT_DIR}/fdlibm/e_atan2/in -o ${ROOT_DIR}/fdlibm/e_atan2/out_covFPFUZZ$i -m none -w -t 5000 -- ${ROOT_DIR}/fdlibm/e_atan2/e_atan2 @@
fi
