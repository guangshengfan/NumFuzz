...
if ! [ $(command llvm-config --version) = "6.0.1" ]; then
    ...
    ${ROOT_DIR}/../../tool/covFPFUZZ/build/bin/afl-gcc -fprofile-arcs -ftest-coverage e_remainder.c -o e_remainder_cov -lm -lgsl -lgslcblas
fi

afl-cov -d out_covFPFUZZ2/ --enable-branch-coverage -c . -e "cat AFL_FILE | ./e_remainder_cov AFL_FILE"
